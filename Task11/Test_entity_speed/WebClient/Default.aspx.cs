...
namespace WebClient
{
    public partial class Default : System.Web.UI.Page
    {
        ...
        public Default()
        {
            _captchaManager = new CaptchaManager(this.Context);
        }
        ...
        private void alert()
        {
            ...
            tableCount.Rows.Add("Заполните необходимые поля.");
        ...
        //- скрыть все поля
        private void hideAllTextboxAndCaptcha()
        {
            //скрыть все поля
            ...
            //скрыть капчу
            _captchaManager.ShowCaptcha(Panel1,false);
        }
        ...
        private void ifAddOil()
        {
            ...
            {
                //если капча указана неверно
                if (!_captchaManager.CheckCaptchaUser(TextBoxCaptcha.Text))
                {
                    Response.Write("<script>alert('Капча указана неверно.')</script>");
                    return;
                }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //добавляем в список радиокнопки при выборе которых должна отображаться капча
            _captchaManager.AddShowRadioButton(RadioAddOil);
            _captchaManager.AddShowRadioButton(RadioAddWell);
            _captchaManager.AddShowRadioButton(RadioClear);
            _captchaManager.AddShowRadioButton(RadioRemoveOilfield);
            _captchaManager.AddShowRadioButton(RadioRemoveWell);
            _captchaManager.AddShowRadioButton(RadioUpdateOilfield);

            //если радиокнопка в списке на показ капчи
            if(_captchaManager.ShowCaptchaOrNot())
                //показать капчу
                _captchaManager.ShowCaptcha(Panel1, true);
            //иначе
            else
                //скрыть капчу
                _captchaManager.ShowCaptcha(Panel1, false);
